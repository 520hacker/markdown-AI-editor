...
import { updateApiKey } from "./api/index";
...
<template>
  <div class="content-root">
    ...
    <ShortCut :shortCutList="globalState.shortCutList" :shortCutTops="globalState.shortCutTops"
      @selection="handleSelection" @shortCutClick="handleShortCutClick" @editShortCut="handleEditShortCut"
      @toppedShortCut="handleToppedShortCut" @addShortCut="globalState.showShortCutAdd = true"></ShortCut>
    <ResultModal :input="globalState.selectionText" :selectionRect="globalState.selectionRect"
      :shortCut="globalState.currentShortCut" :shortCutList="globalState.shortCutList"
      @close="globalState.showResultModel = false" v-if="globalState.showResultModel"></ResultModal>
    <Login v-if="globalState.showLogin" @close="globalState.showLogin = false" @success="handleLoginSuccess"></Login>
    <Chat v-if="globalState.showChat" @close="globalState.showChat = false"
      @updateApiKey="globalState.showApiKeyAdd = true" @logout="handleLogout"></Chat>
    <AddShortCut :shortCut="globalState.curEditShortCut" v-if="globalState.showShortCutAdd"
      @close="globalState.showShortCutAdd = false" @success="handleAddShortSuccess"></AddShortCut>
    <AddApiKey v-if="globalState.showApiKeyAdd" @close="globalState.showApiKeyAdd = false" @submit="handleUpdateApiKey">
    </AddApiKey>
    <TextChat></TextChat>
  </div>
</template>

<style scoped> .content-root {
   position: absolute;
   left: 0;
   top: 0;
   z-index: 2147483647;
