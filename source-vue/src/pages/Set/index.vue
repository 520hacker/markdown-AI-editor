...
import { fetchUserInfo, updateOpenaiApiKey } from "../../api/user";
...
const getUserInfo = () => {
  fetchUserInfo({}).then((response) => {
    console.log('index-fetchUserInfo')
  ...
  <div class="content-root">
    <div class="inner">
      <div class="menu-wrapper">
        <div class="logo">
          <img :src="icon" class="logo-img" style="width: 32px; height: 32px; border-radius: 4px" /><span
            class="logo-title" style="font-size: 20px">AI</span>
        </div>
        <ul class="menu">
          <li class="menu-item" :class="{ menuItemActive: curtab === 'userInfo' }" @click="curtab = 'userInfo'">
            用户信息
          </li>
          <li class="menu-item" :class="{ menuItemActive: curtab === 'shortCut' }" @click="curtab = 'shortCut'">
            快捷指令
          </li>
          <li class="menu-item" :class="{ menuItemActive: curtab === 'template' }" @click="curtab = 'template'">
            提示语模版
          </li>
        </ul>
      </div>
      <div class="content">
        <div class="options-content user-info" v-if="curtab === 'userInfo'">
          ...
          <div class="config-item">
              ...
              <div class="user-card">
                <div class="user-card-right">
                  <div class="user-name">
                    ...
                    <a rel="noreferrer" @click="showApiKeyAdd = true">修改APIKEY</a>
                  </div>
                  <span class="user-email-ucVSRY">APIKEY:
                    {{
            ...
            <AddApiKey v-if="showApiKeyAdd" @close="showApiKeyAdd = false" @submit="handleUpdateApiKey"></AddApiKey>
          </div>
        </div>
        <div class="options-content quick-action" v-if="curtab === 'shortCut'">
          ...
          <div class="quick-action-content">
            <div class="action-list">
              <div class="action-item" v-for="item in shortCutList" :key="item._id">
                <header class="action-item-header">
                  ...
                  <IEdit class="action-edit" @click="handleEditShortCut(item)" />
                </header>
                <article class="action-item-content" :title="item.prompt">
                  {{ item.prompt }}
                </article>
              </div>
              <div class="action-item add-new-action" style="display: flex" @click="handleAddShortCut">
                <IPlus />
                <article class="action-item-content">添加新操作</article>
              </div>
            </div>
          </div>
          <AddShortCut :shortCut="curEditShortCut" v-if="showShortCutAdd" @close="showShortCutAdd = false"
            @success="handleAddShortSuccess"></AddShortCut>
        </div>
        <div class="options-content quick-action" v-if="curtab === 'template'">
          <h2 class="content-title">提示语模版</h2>
          <div class="quick-action-content">
            <div class="action-list">
              <div class="action-item" v-for="(item, index) in templates" :key="index">
                <header class="action-item-header">
                  <span class="action-name-wrapper">
                    <span class="action-name">{{ item.name }}</span>
                  </span>
                </header>
                <article class="action-item-content" :title="item.prompt" style="cursor: pointer"
