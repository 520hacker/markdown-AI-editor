...
const changeShortCut = async(item) => {
    ...
    <div class="lite-window" :style="style">
      <header class="header">
        <div class="header-left">
          <img :src="icon" class="logo-img" style="width: 22px; height: 22px; border-radius: 4px" />
      ...
      <main class="main">
        <div class="action-bar action-bar-lite-window" style="opacity: 1">
          <div class="box">
            <span class="tag-button" :class="{ 'tag-button-active': curShortCut._id === item._id }"
              style="opacity: 1; pointer-events: auto" v-for="(item, index) in frontShortCutList" :key="index"
          ...
          <div class="echo-popover">
            ...
            <div class="content content-animate content-animate-bottom" :class="{ show: showMoreOperate }"
              style="top: 20px; right: 0px">
              <div class="more-content">
                <div class="more-item" v-for="(item, index) in otherShortCutList" :key="index">
                  <component :is="Maps[item.icon]" :width="16" :height="16" />
                  <span class="more-item-text">{{ item.name }}</span>
                </div>
                <div class="more-item manage manage-border" @click="handleGotoManage">
        ...
        <div class="lite-window-input-fold" v-if="!expand" @click="expand = true">
        ...
        <div class="lite-window-input" v-if="expand">
          ...
          <span>
            <Textarea placeholder="来自您选择的文本，可以在触发任何操作之前对其进行修改" v-model:value="inputText"
              :auto-size="{ minRows: 1, maxRows: 5 }"
              class="ant-input content content-preview css ant-input-disabled"></Textarea>
          </span>
          <footer class="footer">
        ...
        <div class="compose-output compose-output-lite-window">
          <div class="title">
            ...
            <div class="operation-group">
              <span title="复制内容" class="operation-item" v-if="resultText && !showCopySuccess"
                v-clipboard:copy="resultText" v-clipboard:success="handleCopySuccess">
                <ICopy :width="16" :height="16"></ICopy>
              </span>
              ...
              <span class="operation-item" @click="genContent">
                <IRefreshRight class="octicon octicon-sync" :width="16" :height="16"></IRefreshRight>
              </span>
            </div>
          </div>
          <Textarea placeholder="" v-model:value="resultText" :auto-size="{ minRows: 3, maxRows: 6 }" disabled
            class="ant-input textarea css"></Textarea>
          <textarea tabindex="2" v-model="resultText" id="myTextarea"></textarea>
        </div>
      </main>
    </div>
